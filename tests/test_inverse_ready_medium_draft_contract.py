...
TASK15_DATASET_CONFIG_PATH = Path("configs/datasets/drafts/inverse_ready_medium_dataset_draft.json")
TASK15_TRAINING_CONFIG_PATH = Path("configs/surrogate/drafts/inverse_ready_medium_training_draft.json")
TASK15_EVALUATION_CONFIG_PATH = Path("configs/surrogate/drafts/inverse_ready_medium_evaluation_draft.json")
TASK15_HANDOFF_PATH = Path("docs/runbooks/inverse_ready_medium_draft_runbook.md")
...
def test_task15_medium_dataset_contract_freezes_budget_and_upstream_dependencies() -> None:
    ...
    assert dataset_config["upstream_pairing_source_contract"] == "configs/datasets/contracts/rmft_anchor_dataset_contract.json"
    assert dataset_config["upstream_bias_contract"] == "configs/datasets/contracts/bias40_probe_dataset_contract.json"
    assert dataset_config["upstream_transport_contract"] == "docs/contracts/transport_domain_contract.md"
    assert dataset_config["upstream_tb_contract"] == "configs/datasets/drafts/tb_pilot_dataset_draft.json"
...
def test_task15_training_and_evaluation_configs_freeze_inverse_ready_medium_contract() -> None:
    ...
    assert evaluation_config["transport_domain_contract"]["source_document"] == "docs/contracts/transport_domain_contract.md"
...
def test_task15_handoff_records_medium_contract_commands_and_review_boundary() -> None:
    ...
    assert "configs/datasets/drafts/inverse_ready_medium_dataset_draft.json" in handoff
    assert "configs/surrogate/drafts/inverse_ready_medium_training_draft.json" in handoff
    assert "configs/surrogate/drafts/inverse_ready_medium_evaluation_draft.json" in handoff
    assert "python scripts/datasets/build_dataset.py --config configs/datasets/drafts/inverse_ready_medium_dataset_draft.json" in handoff
    assert "python scripts/surrogate/train_surrogate.py --config configs/surrogate/drafts/inverse_ready_medium_training_draft.json" in handoff
    assert "python scripts/surrogate/evaluate_surrogate.py --config configs/surrogate/drafts/inverse_ready_medium_evaluation_draft.json" in handoff
