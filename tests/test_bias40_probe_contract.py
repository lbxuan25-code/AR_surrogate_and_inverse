...
TASK14_BIAS40_DATASET_CONFIG_PATH = Path("configs/datasets/contracts/bias40_probe_dataset_contract.json")
TASK14_BIAS40_TRAINING_CONFIG_PATH = Path("configs/surrogate/contracts/bias40_probe_training_contract.json")
TASK14_BIAS40_EVALUATION_CONFIG_PATH = Path("configs/surrogate/contracts/bias40_probe_evaluation_contract.json")
TASK14_BIAS40_HANDOFF_PATH = Path("docs/runbooks/bias40_probe_runbook.md")
...
def test_task14_bias40_handoff_records_prep_only_boundary_and_server_commands() -> None:
    ...
    assert "configs/datasets/contracts/bias40_probe_dataset_contract.json" in handoff
    assert "configs/surrogate/contracts/bias40_probe_training_contract.json" in handoff
    assert "configs/surrogate/contracts/bias40_probe_evaluation_contract.json" in handoff
    assert "python scripts/datasets/build_dataset.py --config configs/datasets/contracts/bias40_probe_dataset_contract.json" in handoff
    assert "python scripts/surrogate/train_surrogate.py --config configs/surrogate/contracts/bias40_probe_training_contract.json" in handoff
    assert "python scripts/surrogate/evaluate_surrogate.py --config configs/surrogate/contracts/bias40_probe_evaluation_contract.json" in handoff
